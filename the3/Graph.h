...
private:
    ...
    long distanceFind(vector<pair<string,long>> &distance, string &country);
    void updateDistance(vector<pair<string,long>> &distance,string &country,long newDistance);
    void updatePrevious(vector<pair<string,string>> &previous, string &country, string &updatedVal);
    string findPrevious(vector<pair<string,string>> &previous, string &country);
    void createPath(list<string> &path, string &country,vector<pair<string,string>> &previous);
    int isVisited(vector<pair<string,int>> &visitedInfo, string &country);
    void markVisited(vector<pair<string,int>> &visitedInfo, string &country);
    bool dfs(vector<bool> &checkCycling, vector<bool> &visitedInfo, string country);
